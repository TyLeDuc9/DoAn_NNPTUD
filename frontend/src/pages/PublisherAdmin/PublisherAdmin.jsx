...
import { createPublisher, deletePublisher } from "../../services/publisherApi";
...
export const PublisherAdmin = () => {
  ...
  const handleDeletePublisher = async (id, name) => {
    const confirmDelete = window.confirm(`Bạn có muốn xóa ${name} không`)
            ...
            {publisher.map((u, idx) => (
              <tr key={u._id} className="text-center bg-white
                ...
                <td className={`${tdClass} text-center`}>
                  <div className='flex justify-center'>
                    <button
                      onClick={() => handleDeletePublisher(u._id, u.name)}
