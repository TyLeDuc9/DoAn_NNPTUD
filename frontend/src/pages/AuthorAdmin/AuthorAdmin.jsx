...
import { deleteAuthor, createAuthor } from "../../services/authorApi";
...
export const AuthorAdmin = () => {
  ...
  const handleDeleteAuthor = async (id, name) => {
    const confirmDelete = window.confirm(`Bạn có chắc muốn xóa tác giả "${name}" không?`);
            ...
            {authors.map((u, idx) => (
              <tr key={u._id} className="text-center bg-white
                ...
                <td className={`${tdClass} text-center`}>
                  <div className='flex justify-center'>
                    <button
                      onClick={() => handleDeleteAuthor(u._id, u.name)}
