...
import { createCategory, deleteCategory } from "../../services/categoryApi";
...
export const CategoryAdmin = () => {
  ...
  const handleDeleteCategory = async (id, name) => {
    const confirmDelete = window.confirm(`Bạn có chắc muốn xóa danh mục "${name}" không?`)
            ...
            {allCategory.map((u, idx) => (
              <tr key={u._id} className="text-center bg-white
                ...
                <td className="border p-2 text-center border-gray-400">
                  <div className='flex justify-center'>
                    <button
                      onClick={() => handleDeleteCategory(u._id, u.name)}
