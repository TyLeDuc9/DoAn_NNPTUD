...
import { deleteRatingBook } from "../../services/ratingApi";
import { ComponentLoading } from "../../components/Loading/ComponentLoading";
import { useLoading } from '../../context/LoadingContext';
export const RatingAdmin = () => {
  // state & search params
  ...
  const handleDelete = async (id) => {
    const confirmDelete = window.confirm("Bạn có muốn xóa tất cả đánh giá của cuốn sách này?");
    if (!confirmDelete) return;
    try {
      await deleteRatingBook(id);
      alert("Đã xóa tất cả đánh giá của cuốn sách!");
        ...
        <table className="border-collapse w-full">
          <thead className="bg-[#386572] text-white text-center">
            <tr>
              ...
              <th className={thClass}>Ngày gần nhất</th>
                    ...
                    >
                      ...
                      {userIdx === 0 && (
                        <>
                          ...
                          <td rowSpan={rowSpan} className={tdClass}>
                            {new Date(r.latestCreatedAt).toLocaleDateString("vi-VN")}
                          </td>
                          <td rowSpan={rowSpan} className={tdClass}>
                            <button
                              className="px-3 py-1 rounded bg-red-500 text-white hover:bg-red-600 transition"
                              onClick={() => handleDelete(r.bookId)}
                            >
